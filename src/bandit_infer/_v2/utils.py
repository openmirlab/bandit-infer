"""Faithful v2 band specifications; kept independent from v1.

The only dependency replacement is the two scalar librosa conversions used by
the upstream musical filterbank, written from their public equations. Reads:
numpy and torch only.
"""
...
def hz_to_midi(hz):
    """librosa-compatible Hz-to-MIDI conversion used by upstream v2."""
    return 12.0 * np.log2(np.asarray(hz) / 440.0) + 69.0


def midi_to_hz(midi):
    """librosa-compatible MIDI-to-Hz conversion used by upstream v2."""
...
def mel_filterbank(n_bands, fs, f_min, f_max, n_freqs):
    raise NotImplementedError("v2 official checkpoints use the musical, not mel, band specification")
