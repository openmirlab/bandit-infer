"""Faithful Bandit v2 graph, adapted only to torch's public STFT functions.

This namespace intentionally has no v1 imports or shared model-layer base.
Reads: sibling v2 graph modules and torch.
"""
...
class _Spectrogram(torch.nn.Module):
    """Public-torch equivalent of the upstream torchaudio Spectrogram module."""
    def __init__(self, n_fft, win_length, hop_length, normalized, center, pad_mode, onesided, window_fn, wkwargs, power=None):
        super().__init__()
...
class _InverseSpectrogram(_Spectrogram):
    """Public-torch equivalent of the upstream torchaudio InverseSpectrogram."""
...
class BaseBandit(nn.Module):
    ...
    def instantitate_spectral(
        ...
        self.istft = _InverseSpectrogram(
            n_fft=n_fft,
            win_length=win_length,
            hop_length=hop_length,
            pad_mode=pad_mode,
