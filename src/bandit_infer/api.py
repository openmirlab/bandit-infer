...
``family=`` selects the model family (``"v1"`` or ``"v2"``), while ``device=``
selects the Torch device for the single shipped runtime. The package exposes
no public compute-framework selector, so callers integrate against the one
release-supported Torch path.

Reads: checkpoints and the Torch device resolver lazily.
"""
...
class BanditSession:
    ...
    def __init__(self, model: str | None = None, *, family: str | None = None, device: str = "auto", weights_dir: Path | None = None, checkpoint_path: Path | None = None, checkpoint_sha256: str | None = None) -> None:
...
def separate(audio: np.ndarray, *, sample_rate: int, model: str | None = None, family: str | None = None, device: str = "auto", weights_dir: Path | None = None, checkpoint_path: Path | None = None, checkpoint_sha256: str | None = None) -> dict[str, np.ndarray]:
    """Separate one input with a fresh session; it intentionally does not cache."""
    with BanditSession(model, family=family, device=device, weights_dir=weights_dir, checkpoint_path=checkpoint_path, checkpoint_sha256=checkpoint_sha256) as session:
